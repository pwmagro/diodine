...
Logo::Logo(xynth::GuiData& g) : guiData(g), button(ABOUT_ID, juce::DrawableButton::ButtonStyle::ImageFitted) {
    ...
    logo_en_svg->replaceColour(logo_en_svg->findColour(100), guiData.getLnf().getTextColor().brighter(0.6).withRotatedHue(0.2));
...
void Logo::paint(juce::Graphics& g) {
    ...
    logo_en_svg->replaceColour(logo_en_svg->findColour(100), guiData.getLnf().getTextColor().brighter(0.6).withRotatedHue(0.2));
    logo_en_svg->setColour(100, guiData.getLnf().getTextColor().brighter(0.6).withRotatedHue(0.2));
